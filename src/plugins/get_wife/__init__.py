from .json_manager import read_json, write_json,remove_json
import asyncio
...
async def _get_wife(bot: Bot, event: GroupMessageEvent):
    ...
    if(cd > cdTime):
        ...
        await get_wife.send(f"臭东西！你换对象的CD还有{cd_msg}", at_sender=True)


nanton = on_command("男同匹配", aliases={"男同配对"}, priority=10, block=True)


@nanton.handle()
async def _nanton(bot: Bot, event: GroupMessageEvent):
    qid = event.get_user_id()
    data = read_json()
    mid = event.message_id
    try:
    ...
    if(cd > cdTime):
        ...
        await nanton.finish(repo_1+MessageSegment.image(url)+repo_2)
