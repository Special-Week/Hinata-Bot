from nonebot.plugin.on import on_endswith
...
pcr_character = on_endswith("是谁", priority=10, block=True)
@pcr_character.handle()
async def _(event:MessageEvent):
    msg = get_message_text(event.json())
    msg = msg[0:-2]
    keys = [k for k, v in CHARA_NAME.items() if msg in v]
    if keys == []:
        await pcr_character.finish(f"字典中没有找到叫{msg}的呢")
    keys = keys[0]
    value = CHARA_NAME[keys][0]
    chara_id = str(keys)
    url = f'https://redive.estertion.win/icon/unit/{chara_id}11.webp'
    ...
    await pcr_character.finish(f"您可能在找的是:{value}"+MessageSegment.image(binary_content))
