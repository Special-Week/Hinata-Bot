...
async def at_send(matcher, user_id, group_id, message=""):
    if group_id is not None:
        await matcher.send(MessageSegment.at(user_id) + message)
    else:
        await matcher.send(message)


async def at_finish(matcher, user_id, group_id, message=""):
    if group_id is not None:
        await matcher.finish(MessageSegment.at(user_id) + message)
    else:
...
class FaceRecognition:
    ...
    async def draw_face_rects(self, img_ori, buf, faces_pos):
        ...
        for face in faces_pos:
            ...
            height = face['height']
            angle = -face['rotation']
            font = ImageFont.truetype("simfang.ttf", int(width / 5))
